...
from gaphor.UML.event import DerivedChangeEvent, DerivedSetEvent, DerivedAddEvent, DerivedDeleteEvent
...
from gaphor.UML.interfaces import IElementChangeEvent, IAssociationChangeEvent, IAssociationSetEvent
        ...
        if event.property in self.subsets:
            ...
            if self.upper == 1:
                ...
                if IAssociationSetEvent.providedBy(event):
                    ...
                    self.logger.error('Don''t know how to handle event ' + str(event) + ' for derived union')
    ...
    def _association_changed(self, event):
        if event.property is self.original and isinstance(event.element, self.decl_class):
            ...
            if IAssociationSetEvent.providedBy(event):
                ...
                self.logger.error('Don''t know how to handle event ' + str(event) + ' for redefined association')
