...
class HelpService(object):
    ...
    def about(self):
        logo_file =  os.path.join(pkg_resources.get_distribution('dabbler').location, 'dabbler', 'ui', 'pixmaps', 'logo.png')
        logo = gtk.gdk.pixbuf_new_from_file(logo_file)
        version = Application.distribution.version
        about = gtk.Dialog(_('About Dabbler'), self.main_window.window, gtk.DIALOG_MODAL, (gtk.STOCK_OK, gtk.RESPONSE_OK))
        ...
        add_label('Dabbler is written by:\n'
                  'Dan Yeaw\n'
                  'Arjan Molenaar\n'
                  'Artur Wroblewski\n'
                  'Jeroen Vloothuis')
