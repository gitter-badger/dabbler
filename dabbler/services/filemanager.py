...
from dabbler.interfaces import IService, IActionProvider, IServiceEvent
from dabbler.core import _, inject, action, build_action_group
from dabbler.storage import storage, verify
from dabbler.UML import uml2
from dabbler.misc.gidlethread import GIdleThread, Queue, QueueEmpty
from dabbler.misc.errorhandler import error_handler
from dabbler.misc.xmlwriter import XMLWriter
from dabbler.ui.statuswindow import StatusWindow
from dabbler.ui.questiondialog import QuestionDialog
...
class FileManager(object):
    """
    The file service, responsible for loading and saving Dabbler models.
    """
    ...
    def init(self, app):
        ...
        Dabbler files is then updated in the file menu."""
    ...
    def load(self, filename):
        """Load the Dabbler model from the supplied file name.  A status window
    ...
    def _open_dialog(self, title):
        ...
        filter.set_name("Dabbler models")
        filter.add_pattern("*.dabbler")
    ...
    def action_new_from_template(self):
        ...
        filters = [{'name':_('Dabbler Models'), 'pattern':'*.dabbler'},\
                   {'name':_('All Files'), 'pattern':'*'}]

        file_dialog = FileDialog(_('New Dabbler Model From Template'),\
                                 filters = filters)
    ...
    def action_open(self):
        ...
        file_dialog = FileDialog(_('Open Dabbler Model'),\
                                 filters = filters)
    ...
    def action_save_as(self):
        ...
        file_dialog = FileDialog(_('Save Dabbler Model As'),\
                                 action='save',\
                                 filename=self.filename)
