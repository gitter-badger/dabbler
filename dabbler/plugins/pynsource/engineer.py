...
from dabbler.diagram.interfaces import IConnect
...
class Engineer(object):
    """
    The Engineer class will create a Dabbler model based on a list of Python
    files.
    """
    ...
    def _create_class(self, clazz, name):
        ...
        ci.subject = c
        clazz.dabbler_class = c
        clazz.dabbler_class_item = ci
        ...
        if not clazz.ismodulenotrealclass:
            for superclassname in clazz.classesinheritsfrom:
                if superclassname in BASE_CLASSES:
                    continue
                try:
                    superclass = self.parser.classlist[superclassname].dabbler_class
                    superclass_item = self.parser.classlist[superclassname].dabbler_class_item
                ...
                # gen.specific = clazz.dabbler_class
                geni = self.diagram.create(items.GeneralizationItem)
                # geni.subject = gen

                self.connect(geni, geni.tail, clazz.dabbler_class_item)
                ...
                # clazz.dabbler_class_item.connect_handle(geni.handles[-1])
                # adapter = component.queryMultiAdapter((clazz.dabbler_class_item, geni), IConnect)
                # assert adapter
    ...
    def _create_methods(self, clazz):
        for adef in clazz.defs:
            ...
            op.name = adef
            clazz.dabbler_class.ownedOperation = op

    def _find_class_by_name(self, classname):
        try:
            superclass = self.parser.classlist[classname].dabbler_class
            superclass_item = self.parser.classlist[classname].dabbler_class_item
        except KeyError as e:
    ...
    def _create_attribute(self, clazz, attr):
        ...
        if tail_type:
            ...
            head_type = clazz.dabbler_class
            head_type_item = clazz.dabbler_class_item
            ...
            clazz.dabbler_class.ownedAttribute = prop
        # print many
        import pprint
        pprint.pprint(attr)
        # print dir(attr)
