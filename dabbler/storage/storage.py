...
"""
Load and save Dabbler models to Dabblers own XML format.
...
from dabbler import diagram
from dabbler.UML import uml2, modelfactory
from dabbler.UML.collection import collection
from dabbler.UML.elementfactory import ElementChangedEventBlocker
from dabbler.application import Application, NotInitializedError
from dabbler.diagram import items
from dabbler.i18n import _
from dabbler.storage import parser

__all__ = ['load', 'save']

FILE_FORMAT_VERSION = '3.0'
NAMESPACE_MODEL = 'http://dabbler.sourceforge.net/model'
...
def save_generator(writer, factory):
    ...
    dabbler.misc.xmlwriter.XMLWriter instance.
    """
    ...
    def save_element(name, value):
        """
        Save attributes and references from items in the dabbler.UML module.
        A value may be a primitive (string, int), a dabbler.UML.collection
    ...
    def save_canvasitem(name, value, reference=False):
        """
        Save attributes and references in a dabbler.diagram.* object.
    ...
    writer.startElementNS((NAMESPACE_MODEL, 'dabbler'), None,
                          {(NAMESPACE_MODEL, 'version'): FILE_FORMAT_VERSION,
                           (NAMESPACE_MODEL, 'dabbler-version'): Application.distribution.version})
    ...
    # writer.endElement('dabbler')
    writer.endElementNS((NAMESPACE_MODEL, 'dabbler'), None)
...
def load_elements_generator(elements, factory, dabbler_version=None):
    """
    ...
    version_0_6_2(elements, factory, dabbler_version)
    version_0_7_2(elements, factory, dabbler_version)
    version_0_9_0(elements, factory, dabbler_version)
    version_0_14_0(elements, factory, dabbler_version)
    version_0_15_0_pre(elements, factory, dabbler_version)
    version_0_17_0(elements, factory, dabbler_version)
    ...
    # Fix version inconsistencies
    version_0_5_2(elements, factory, dabbler_version)
    version_0_7_1(elements, factory, dabbler_version)
    version_0_15_0_post(elements, factory, dabbler_version)
...
def load_generator(filename, factory):
    ...
    try:
        ...
        dabbler_version = loader.dabbler_version
    ...
    try:
        ...
        try:
            for percentage in load_elements_generator(elements, factory, dabbler_version):
                if percentage:
...
def version_lower_than(dabbler_version, version):
    """
    if version_lower_than('0.3.0', (0, 15, 0)):
       ...
    """
    parts = dabbler_version.split('.')
    try:
...
def version_0_15_0_pre(elements, factory, dabbler_version):
    """
    ...
    if version_lower_than(dabbler_version, (0, 14, 99)):
        # update associations
...
def version_0_15_0_post(elements, factory, dabbler_version):
    """
...
def convert_tagged_value(element, elements, factory):
    ...
    UML 2.2 model (since Dabbler version 0.15).
...
def version_0_17_0(elements, factory, dabbler_version):
    """
    ...
    print('version', dabbler_version)
    if version_lower_than(dabbler_version, (0, 17, 0)):
...
def version_0_14_0(elements, factory, dabbler_version):
    """
    Fix applied stereotypes UML metamodel. Before Dabbler 0.14.0 applied
    ...
    """
    import uuid
    if version_lower_than(dabbler_version, (0, 14, 0)):
...
def version_0_9_0(elements, factory, dabbler_version):
    """
    ...
    This function is called before the actual elements are constructed.
    """
    if version_lower_than(dabbler_version, (0, 9, 0)):
...
def version_0_7_2(elements, factory, dabbler_version):
    """
    ...
    if version_lower_than(dabbler_version, (0, 7, 2)):
        for elem in elements.values():
            try:
...
def version_0_7_1(elements, factory, dabbler_version):
    """
    ...
    if version_lower_than(dabbler_version, (0, 7, 1)):
        log.info('Fix navigability of Associations (file version: %s)' % dabbler_version)
        for elem in elements.values():
            try:
...
def version_0_5_2(elements, factory, dabbler_version):
    """
    Before version 0.5.2, the wrong memberEnd of the association was
    holding the aggregation information.
    """
    if version_lower_than(dabbler_version, (0, 5, 2)):
        log.info('Fix composition on Associations (file version: %s)' % dabbler_version)
        for elem in elements.values():
            try:
                if elem.type == 'Association':
                    a = elem.element
