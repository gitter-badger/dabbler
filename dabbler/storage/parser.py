...
in the dabbler.UML module. Elements also have a unique ID, by which they are
...
class name in the dabbler.diagram module.
...
from dabbler.misc.odict import odict
...
XMLNS='http://dabbler.sourceforge.net/model'

class ParserException(Exception):
    pass

# Loader state:
[ ROOT,         # Expect 'dabbler' element
...
class DabblerLoader(handler.ContentHandler):
    ...
    def startElement(self, name, attrs):
        ...
        # The <dabbler> tag is the toplevel tag:
        elif state == ROOT and name == 'dabbler':
            assert attrs['version'] in ('3.0',)
            self.version = attrs['version']
            self.dabbler_version = attrs.get('dabbler-version')
            if not self.dabbler_version:
                self.dabbler_version = attrs.get('dabbler_version')
...
def parse_generator(filename, loader):
    ...
    """
    assert isinstance(loader, DabblerLoader), 'loader should be a DabblerLoader'
...
def parse_file(filename, parser):
    ...
    should be a DabblerLoader instance.  The filename parameter can be an
