...
"""
This module allows Dabbler to be launched from the command line.
...
def launch(model=None):
    ...
    The file_manager service is used here to load a Dabbler model if one was
    specified on the command line.  Otherwise, a new model is created and
    the Dabbler GUI is started."""
...
def main():
    """Start Dabbler from the command line.  This function creates an option
    parser for retrieving arguments and options from the command line.  This
    includes a Dabbler model to load.
    ...
    if options.profiler:
        ...
        cProfile.run('import dabbler; dabbler.launch()', 'dabbler.prof')

        profile_stats = pstats.Stats('dabbler.prof')
...
six.moves.builtins.__dict__['log'] = logging.getLogger('Dabbler')
