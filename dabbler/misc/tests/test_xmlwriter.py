...
class XMLWriterTestCase(unittest.TestCase):
    ...
    def test_elements_ns_default(self):
        ...
        xml_w = XMLWriter(w)
        xml_w.startDocument()
        xml_w.startPrefixMapping(None, 'http://dabbler.devjavu.com/schema')
        xml_w.startElementNS(('http://dabbler.devjavu.com/schema', 'foo'), 'qn', {})
        xml_w.endElementNS(('http://dabbler.devjavu.com/schema', 'foo'), 'qn')

        xml = """<?xml version="1.0" encoding="%s"?>\n<foo xmlns="http://dabbler.devjavu.com/schema"/>""" % sys.getdefaultencoding()
        assert w.s == xml, w.s
    ...
    def test_elements_ns_1(self):
        ...
        xml_w = XMLWriter(w)
        xml_w.startDocument()
        xml_w.startPrefixMapping('g', 'http://dabbler.devjavu.com/schema')
        xml_w.startElementNS(('http://dabbler.devjavu.com/schema', 'foo'), 'qn', {})
        xml_w.endElementNS(('http://dabbler.devjavu.com/schema', 'foo'), 'qn')

        xml = """<?xml version="1.0" encoding="%s"?>\n<g:foo xmlns:g="http://dabbler.devjavu.com/schema"/>""" % sys.getdefaultencoding()
        assert w.s == xml, w.s
